...
<div class="wrapper">
  <h1 style="font-size:1.6rem;padding:2rem 0 0.5rem;font-weight:800;">{title}</h1>
  <p style="color:var(--text-dim);font-size:0.875rem;padding-bottom:1rem;">
    Full security assessment report for <code>{domain}</code>
  </p>
  ...
  <section id="dashboard">
    ...
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-bottom:1.5rem;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.75rem;">
        ...
        <span style="font-family:var(--mono);font-size:1.5rem;font-weight:800;color:{risk_color};">
          {risk_score}/100
        </span>
      </div>
      <div class="risk-bar">
      ...
      <div style="margin-top:0.75rem;font-size:0.85rem;color:var(--text-dim);">{risk_label}</div>
    </div>
...
class HTMLReportGenerator:
    ...
    def generate(self, domain: str, results: dict, title: str = "") -> Path:
        ...
        html = HTML_TEMPLATE.format(
            title=title or f"Shaheen 3 — {domain}",
            domain=domain,
            ...
            stat_cards=self._stat_cards(results),
            whois_content=self._render_whois(results),
            subdomain_content=self._render_subdomains(results),
            dns_content=self._render_dns(results),
            cert_content=self._render_certs(results),
            email_content=self._render_emails(results),
            nmap_content=self._render_nmap(results),
            http_content=self._render_http(results),
            vuln_content=self._render_vuln(results),
            web_content=self._render_web(results),
            waf_content=self._render_waf(results),
            exploit_content=self._render_exploits(results),
            screenshot_content=self._render_screenshots(results),
        )
